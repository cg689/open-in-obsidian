...
- **The first double-click in a new folder takes about 1.2 s longer — but only while Obsidian is already running.** A running Obsidian learns about the freshly created junction through its file watcher, and the protocol resolves paths against its in-memory index, so without a grace period that first open can miss. When Obsidian is closed no wait is needed — it rescans the disk when it loads the vault, and the cold-start time is Obsidian's own
- **The bridge vault keeps itself small.** Every run prunes two kinds of links: dangling ones (whose target has vanished — previously a single dangling link made every later vault-external file fall back to the fallback editor), and links whose target would nest the vault inside itself (e.g. mounting `C:\Users` wholesale while the vault lives under `C:\Users\...` — Obsidian's indexer then walks that recursion on every vault load, re-indexing the mounted content over and over, which shows up as minute-long loads or apparent hangs; those folders fall back to the editor). It also keeps only the 10 most recently used mounts and evicts older ones — removing a junction never touches the real folder, so the vault cannot grow forever and slow every load down
