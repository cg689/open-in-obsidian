...
- **零依赖**：不需要下载任何东西，用 Windows 自带的 .NET Framework 编译器现场编译一个转发程序（约 740 行源码，含 vault 检测、桥接挂载与回落逻辑）——源码就在 `src/`，装的是什么一目了然
...
- **首次在某个新文件夹里双击会慢约 0.4 秒**。程序在等 Obsidian 的文件监听器认出刚建的联接，否则那一次打开可能落空
